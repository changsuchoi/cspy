...
psf=True
# se_sub(im,psf=psf)
def se_sub(subim,psf=psf,dual=False,det=False):
	im=subim[2:]
	PSCALE=fits.getheader(im)['PSCALE']
	fwhm_pix=fits.getheader(im)['FWHM_PIX']
	opt_ap=fits.getheader(im)['OPT_AP']
...
def se_zp_sub(im,ext='.set'):
	from astropy.table import Table
	from astropy import units as u
	from astropy.table import Column
	fn = os.path.splitext(im)[0]
	hdr=fits.getheader(im[2:])
	sefcat=ascii.read(fn+ext)
	#print('Making final catalog',fn+'.dat')
	sefcat['MAG_AUTO']=sefcat['MAG_AUTO']+hdr['ZP_AUTO']
	sefcat['MAGERR_AUTO']=np.sqrt(sefcat['MAGERR_AUTO']**2 + hdr['ZPE_AUTO']**2)
...
def field_target_phot(im, tra, tdec):
	hdr=fits.getheader(im)
	setarget(im)
	tbl=se_zp(im)
	trow, tsep = targetfind(tra, tdec,
					refra=tbl['ALPHA_J2000'], refdec=tbl['DELTA_J2000'], sep=2.0)
	t = Time(hdr['DATE-OBS'], format='isot', scale='utc').mjd
	body1=str(round(t,7))+'\t'+im.split('-')[1]+'\t'+ im.split('-')[5]+'\t'
	body2=''
	for j in tblcols: body2+=str(tbl[trow][j])+'\t'
	body3=''
	for h in hdrkey: body3+=str(hdr[h])+'\t'
	body=body1+body2+body3[:-1]+'\n'
	return body
...
def trim(inim, positions=pos, sizes=sz):
	...
	hdr = hdu.header
	w = WCS(hdu.header)
	outim=os.path.splitext(inim)[0]+'_'+str(sizes[0])+'min_stamp.fits'
...
def fitplot_target_stamp(inim,tra,tdec,sizes):
	im=os.path.splitext(inim)[0]+'_'+str(sizes[0])+'min_stamp.fits'
	import matplotlib.pyplot as plt
	from astropy.wcs import WCS
	...
	ax.scatter(tra,tdec, transform=ax.get_transform('fk5'),
				s=30, edgecolor='yellow',facecolor='none')
	...
	ax.invert_yaxis()
	fig.colorbar(img)
	plt.savefig(os.path.splitext(inim)[0]+'_'+str(sizes[0])+'min_stamp.png')
	plt.close()
...
sz=sizes
head=''
for j in tblcols: head+=j+'\t'
for h in hdrkey: head+=h+'\t'
resulthead= '#MJD'+ '\t'+'OBSERVATORY'+'\t'+'FILTER'+'\t'+head[:-1]+'\n'
os.getcwd().split('/')[-1]
tra,tdec=161.6715913,13.7165212
f=open(targetname+'-'+os.getcwd().split('/')[-2]+'-'+os.getcwd().split('/')[-1]+'.dat','w')
f.write(resulthead)
for im in tmp:
	body=field_target_phot(im, tra, tdec)
	f.write(body)
	#trim(im, positions=pos, sizes=sz)
